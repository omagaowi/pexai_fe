...
const MasonryGrid: React.FC<IMasonryGridProps> = ({
  ...
  const columnWidth = Math.floor(containerSize.width / (columns + 0.1));
  ...
  useEffect(() => {
    if (
      gridRef.current &&
      totalHeight + 70 < containerSize.height &&
      !isLoading
    ) {
      onNeedMore();
    }
          ...
          {columnWidth > 0 ? (
            <Link
              to={
                photo.status == "pending"
                  ? `/remix/${photo.chat_id}`
                  : `/photo/${photo.pexai_id}`
              }
              onClick={() => {
                if (photo.origin != "pexai") {
            ...
            >
              {!photo.status || photo?.status == "completed" ? (
                ...
                <div
                  className={`w-[${columnWidth}] aspect-square relative bg-[] rounded`}
                >
                  <Skeleton className="h-full w-full absolute top-0 left-0 bg-[#2e2e2e] rounded-lg" />
                  <div className="h-full w-full absolute top-0 left-0 bg-[#2e2e2e] rounded-lg">
                    <AiSparkles />
                  </div>
                </div>
              )}
